  ...
  const fetchblursucts = async () => {
    try {
      const response = await fetch(
        "https://yogoblastpanel-3.onrender.com/pages/cartDisplay",
        {
      ...
      data.message == "Invalid or expired token"
        ? alert("please login first to continue with cart")
        : setproducts(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const deleteProduct = async (product) => {
    ...
    try {
      const res = await axios.post(
        "https://yogoblastpanel-3.onrender.com/pages/update",
        deleteData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
          ...
          products.map((product) => (
            <div key={product.products_id} className="cart-products">
              ...
              <div className="cart-buttons">
                <button
                  onClick={() => {
                    navigate(`/home/product/${product.products_id}`);
                  }}
                >
                  View
                </button>
