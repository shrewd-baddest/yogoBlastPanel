...
const Display = () => {
    ...
    const n_image = async () => {
      try {
        const res = await fetch(
          "https://yogoblastpanel-3.onrender.com/pages/home",
        );
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      ...
      gsap.utils.toArray(".detail-gsap"),
      {
        opacity: 0,
        x: 100,
      },
  ...
  return (
    <>
      <div style={{ paddingLeft: "8%", paddingRight: "8%" }} ref={blurs}>
        {(details || []).map((detail) => (
          ...
          >
            ...
            <div className="detail-gsap">
              <h2 className="des">{detail.description}</h2>
              ...
              <div className="Pay_buttons">
                <button
                  className="cart"
                  onClick={() => addToCart(detail.products_id)}
                >
                  Add to Cart
                </button>
                <button className="pay_now" onClick={() => Buy(detail)}>
                  Pay Now
                </button>
              </div>
            </div>
        ...
        <h3 className="gsap-new" style={{ opacity: "1" }}>
        ...
        <div className="new">
          {newImages.map((newImage) => (
            <div key={newImage.products_id} className="newImage">
              <Link
                to={`/home/product/${newImage.products_id}`}
                className="nlinks"
                style={{ textDecoration: "none" }}
              >
                <img src={newImage.image_url} loading="lazy" />
                <p>
                  {" "}
                  {newImage.products_name} {newImage.weight_ml} ml
                </p>
        ...
        <div className="like">
          {imagesLike.map((imagePresent) => (
            <div key={imagePresent.products_id} className="likedImage">
              <Link
                to={`/home/product/${imagePresent.products_id}`}
                className="llinks"
                style={{ textDecoration: "none" }}
              >
                <img src={imagePresent.image_url} loading="lazy" />
                <p>
                  {" "}
                  {imagePresent.products_name} {imagePresent.weight_ml} ml
                </p>
      ...
      <div className="pay_form" ref={pay}>
        <div>
          <button
            ...
            className="close"
            onClick={() => {
              pay.current.style.display = "none";
              blurs.current.style.filter = "blur(0px)";
            }}
          >
            x
          </button>
          <p className="cart-psg">
            we guarantee a secure money transaction and 100% refund
          </p>
          <p className="cart-price">
            Ksh:{price != null ? price.toFixed(2) : "0.00"}
          </p>
        </div>
        <form
          onSubmit={(e) => {
